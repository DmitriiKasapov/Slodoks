...
[2026-08-03] [каркас] Шапка, подвал и меню свёрстаны с нуля, без стилей шаблона Consal. Шапка: логотип с квадратом-маркой в акцентном синем, меню, кнопка «Записаться», бургер, липкое поведение с тенью после прокрутки. Одна разметка меню на все экраны: на широких строка, ниже 926px панель под шапкой; закрывается по Escape с возвратом фокуса, по клику вне и при переходе на широкий экран. Состояния пунктов: наведение — синий текст и синяя полоса, активный — синий текст и красная полоса, которая остаётся красной и под курсором; на мобильном полоса вертикальная слева. Подвал: четыре колонки (бренд, меню, услуги, кнопка) плюс строка копирайта, ссылки с выезжающим подчёркиванием. Область меню одна — Primary, выводится и в шапке, и в подвале, в подвале обрезана до одного уровня через depth — выполнено

[2026-08-03] [стили] Общие стили тела: DM Sans, 16px, вес 400, интерлиньяж 1.3, белый фон, цвет текста #6E6F70. Каркас страницы прижимает подвал вниз на коротких страницах (flex-колонка, main растягивается, высота через dvh для мобильных). Отступы между секциями вынесены в утилиту `.section`: 30px на мобильном, 60px от 768px, 100px от 926px. Дублирование общих стилей убрано: у заголовков h1–h6 общая часть вынесена в одно правило, в разметке заголовочные классы вместо ручного набора утилит. Правило «общие стили не дублируются» записано в CLAUDE.md темы — выполнено

[2026-08-03] [шрифты] DM Sans подключён локально для основного текста (400, 500, 700), Montserrat остался для заголовков. Временное подключение Google Fonts для примерки DM Sans, Fredoka и Urbanist убрано вместе с preconnect. Найдена и исправлена ошибка конфига: в dev-режиме шрифты не грузились, потому что Vite отдавал относительные пути, а страница живёт на другом порту — вернул server.origin, порт снова закреплён через strictPort — выполнено
